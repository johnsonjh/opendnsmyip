...
export TEST_FLAGS="-count=1 -covermode=atomic -cpu=1 -parallel=1 -race -trimpath"
...
(
	...
	go test -v ${TEST1_TAGS:?${unset:?}} ${TEST_FLAGS:?${unset:?}} -bench=. ${GOC_TARGETS} -coverprofile gocov_report_opendnsmyip1.profile |
...
if [ -x "${HOME}/.opendnsmyip.cov.local" ]; then
	printf '%s\n' "Local script started"
	("${HOME}/.opendnsmyip.cov.local")
	printf '%s\n' "Local script ended"
fi

mkdir -p ./cov &&
	mv -f ./gocov_report_opendnsmyip* ./cov &&
